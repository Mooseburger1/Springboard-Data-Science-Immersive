...
def compute_cost(AL, Y, lambd, caches):
    ...
    cost = - np.sum(np.sum(Y * np.log(AL)) + np.sum(np.sum((1 - Y) * np.log(1 - AL)))) / m
    reg = 0
    layers = len(caches)
    ...
    regularization = (lambd / (m*2)) * reg 
    
    cost = np.squeeze(cost) + regularization     # To make sure your cost's shape is what we expect (e.g. this turns [[17]] into 17).
    assert(cost.shape == ())
...
def accuracy(AL, Y, train = False):
    ...
    if train is True:
        ...
        sns.heatmap(conf_matrix, annot=True, fmt = 'd', xticklabels = np.unique(Y.T.argmax(axis=1)),  yticklabels = np.unique(Y.T.argmax(axis=1)), cmap = 'coolwarm')
